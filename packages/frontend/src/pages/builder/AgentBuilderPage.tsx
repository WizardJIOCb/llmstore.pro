...
- Искать статьи на DTF по игре, теме, компании, человеку или ключевым словам через инструмент dtf-search-articles
...
- Если период не указан, ищи по теме за всё доступное время: сначала dtf-search-articles с period = "all", limit = 10; если результатов мало, дополнительно используй dtf-popular-feed и dtf-latest-feed
...
- В списках статей сохраняй сортировку "сначала самые новые": не ставь старые статьи выше новых ради популярности, реакций или комментариев
- При перечислении статей используй компактный Telegram-friendly Markdown: каждая статья отдельным блоком с пустой строкой между блоками
- Формат блока: "• **Заголовок**\nАвтор: ...\nДата: ДД.ММ.ГГГГ\n[Читать на DTF](URL)\nКомментарии: N, Реакции: N"
- Не вставляй голые длинные URL; всегда используй Markdown-ссылку "[Читать на DTF](URL)"
- Не перечисляй полный список реакций; показывай только общий счётчик реакций
- Если инструмент вернул дату published_at, показывай только дату в формате ДД.ММ.ГГГГ, без времени и ISO-строки
- Если найдено больше 7 статей, покажи 7 самых новых среди релевантных и коротко упомяни, что есть ещё
- При пересказе выделяй ключевые моменты
- Если пользователь просит последние/свежие/актуальные новости по теме или игре, используй dtf-search-articles по этой теме и показывай новые сверху; если тема не указана, получи общую ленту через dtf-latest-feed`,
  runtime_config: {
    max_iterations: 6,
    temperature: 0.3,
    max_tokens: 4096,
...
export function AgentBuilderPage() {
  ...
  const getDtfToolIds = () => {
    ...
    return tools
      .filter((t) => (
        t.slug === 'dtf-latest-feed'
        || t.slug === 'dtf-search-articles'
        || t.slug === 'dtf-popular-feed'
        || t.slug === 'dtf-article-fetch'
      ))
      .map((t) => t.id);
  };
