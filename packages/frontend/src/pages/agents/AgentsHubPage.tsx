...
export function AgentsHubPage() {
  ...
  const { data: chatAgents, isLoading: chatAgentsLoading } = useChatAgents();
  const createChat = useCreateChat();
  const myAgentsCount = myAgents?.length ?? 0;
  const discoverAgents = useMemo(() => {
    const query = searchDiscover.trim().toLowerCase();
    return (chatAgents ?? [])
      .filter((agent) => {
  ...
  }, [chatAgents, searchDiscover]);
  const discoverAgentsCount = discoverAgents.length;
  ...
  const handleStartChat = async (agentId: string) => {
    ...
    navigate(`/chats?chat=${created.id}`);
  };
      ...
      {tab === 'my' && (
        <>
          <div className="mb-6 rounded-xl border bg-slate-50 p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Здесь вы управляете своими агентами: запускаете, редактируете, удаляете и смотрите их статистику.
            </p>
          </div>
      ...
      {tab === 'search' && (
        <>
          <div className="mb-6 rounded-xl border bg-slate-50 p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Здесь собраны все агенты, доступные для режима чата с агентом: ваши собственные и публичные агенты других пользователей.
            </p>
          </div>

          <div className="mb-6">
            <Input
              placeholder="Искать по названию агента, описанию или автору..."
          ...
          {chatAgentsLoading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">Подходящие агенты не найдены</p>
            </div>
          ) : (
            <div className="space-y-3">
                  ...
                  <Card key={agent.id}>
                    ...
                    <CardContent>
                      <div className="mb-3 space-y-1">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Популярность: {agent.total_runs.toLocaleString('ru-RU')} запуск.
                        </div>
                        {hasUsage(myStats) && (
                          <StatsRow label="Моё:" stats={myStats} />
                        )}
