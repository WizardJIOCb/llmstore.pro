...
export function CatalogListPage({ type, filterLanguage, title, articleMode = false }: CatalogListPageProps) {
  ...
  const pageTitle = title ?? (type ? (contentTypeLabels[type] ?? type) : 'Статьи');
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6 space-y-3">
        <div className="flex items-center gap-3">
          <SearchBar onSearch={setSearch} placeholder={`Поиск в ${pageTitle.toLowerCase()}...`} />
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <CatalogCard
                key={item.id}
                item={item}
                hrefOverride={articleMode ? `/article/${item.slug}` : undefined}
              />
            ))}
          </div>
