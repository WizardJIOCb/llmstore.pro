...
export const aliceWebhookLogs = pgTable('alice_webhook_logs', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').references(() => users.id, { onDelete: 'set null' }),
  chat_id: uuid('chat_id').references(() => chatConversations.id, { onDelete: 'set null' }),
  yandex_skill_user_id: varchar('yandex_skill_user_id', { length: 255 }),
  yandex_application_id: varchar('yandex_application_id', { length: 255 }),
  ...
  original_utterance: text('original_utterance'),
  request_json: jsonb('request_json').$type<Record<string, unknown> | null>(),
  response_json: jsonb('response_json').$type<Record<string, unknown> | null>(),
  response_text: text('response_text'),
  response_status_code: integer('response_status_code').notNull().default(200),
  ...
  bonus_granted: boolean('bonus_granted'),
  ip_address: varchar('ip_address', { length: 255 }),
  user_agent: text('user_agent'),
  duration_ms: integer('duration_ms'),
  created_at: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => [
  index('alice_webhook_logs_created_idx').on(table.created_at),
  index('alice_webhook_logs_user_id_idx').on(table.user_id),
  index('alice_webhook_logs_chat_id_idx').on(table.chat_id),
  index('alice_webhook_logs_skill_user_id_idx').on(table.yandex_skill_user_id),
  index('alice_webhook_logs_application_id_idx').on(table.yandex_application_id),
