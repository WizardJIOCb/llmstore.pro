...
- в списках статей сохраняй сортировку "сначала самые новые": не переставляй более старые статьи выше новых ради популярности, реакций или комментариев;
...
- если пользователь просит последние, свежие, новые или актуальные новости по конкретной теме/игре/ключевому слову, используй dtf-search-articles по этой теме и показывай результаты по дате от новых к старым; если тема не указана, вызывай dtf-latest-feed;
...
- если найдено больше 7 статей, показывай 7 самых новых среди релевантных и коротко допиши, сколько ещё есть в найденной выборке; исключение: пользователь явно попросил конкретное число статей;
...
const DTF_AGENT_VERSION_NUMBER = 11;
